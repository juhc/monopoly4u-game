...
class JackpotField(AbstractField):
    ...
    def on_enter(self, player_name: str) -> FieldAction:
        return FieldAction(
            ...
            description=f"Игрок {player_name} должен согласиться и заплатить {self.__cost} или отказаться от игры в джекпот",
            value=self.__cost,
        )
    ...
    def on_accept(self, player_name: str, dice_values: list[int]) -> FieldAction:
        ...
        if dice_value in dice_values:
            ...
            return FieldAction(
                ...
                description=f"Игрок {player_name} побеждает и получает {value}",
                value=value,
            )
        else:
            return FieldAction(
                action_type=ActionType.DO_NOTHING,
                description=f"Игрок {player_name} проигрывает и ничего не получает",
            )
    ...
    def on_decline(self, player_name: str) -> FieldAction:
        return FieldAction(
            action_type=ActionType.DO_NOTHING,
            description=f"Игрок {player_name} отказывается от игры в джекпот",
        )
